...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.variable_scope('embedding_layer'):
        with tf.device('/cpu:0'):
          ...
          inputs = tf.nn.embedding_lookup(embedding, ids=self.input_placeholder)
          inputs = tf.reshape(inputs, [-1, self.config.num_steps, self.config.embed_size])
          inputs = tf.unstack(inputs, self.config.num_steps, axis=1)
          ### END YOUR CODE
          return inputs
  ...
  def add_loss_op(self, output):
    ...
    loss = sequence_loss(logits=output, targets=tf.cast(self.labels_placeholder, dtype=tf.int32),
                         weights=weights)
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN_layer') as scope:
        ...
        I = tf.get_variable('I', shape=[self.config.embed_size, self.config.hidden_size],
                            initializer=tf.truncated_normal_initializer(stddev=0.1))
        b_1 = tf.get_variable('b_1', shape=[self.config.hidden_size], initializer=tf.constant_initializer(0.0))
        rnn_outputs = []
        h = tf.sigmoid(tf.matmul(self.initial_state, H) + tf.matmul(inputs[0], I) + b_1)
        rnn_outputs.append(h)
        # scope.reuse_variables()
        for i in range(1, self.config.num_steps):
            h = tf.sigmoid(tf.matmul(h, H) + tf.matmul(inputs[i], I) + b_1)
            rnn_outputs.append(h)
        self.final_state = h

    with tf.variable_scope('output_drop'):
  ...
  def run_epoch(self, session, data, train_op=None, verbose=10):
    ...
    for step, (x, y) in enumerate(
      ...
      l= session.run(
          [self.calculate_loss], feed_dict=feed)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in range(stop_length):
    ### YOUR CODE HERE
    y_pred, state = session.run([model.predictions, model.final_state],
                                feed_dict={model.input_placeholder: [tokens[-config.num_steps:]],
