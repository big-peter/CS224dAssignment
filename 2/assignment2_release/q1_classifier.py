...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    """
    W = tf.Variable(initial_value=tf.zeros([self.config.n_features, self.config.n_classes], dtype=tf.float32))
    b = tf.Variable(initial_value=tf.zeros([self.config.n_classes], dtype=tf.float32))
